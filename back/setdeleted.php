...
if ($_SERVER["REQUEST_METHOD"] === "POST") 
{
        $formObject = json_decode(file_get_contents("php://input"), false);
                ...
                if ($type == "D") {
                        $sql = "DELETE FROM tbl_linetoken_detail WHERE keycode = :keycode";
                }

                if ($type == "S") {
                        $sql = "DELETE FROM tbl_linetoken_summary WHERE keycode = :keycode";
                }
                ...
                try {
                        ...
                        $response = array(
                                'status' => 1,
                                'text' => "The key has been deleted, successfully."
                        );
