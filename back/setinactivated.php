...
if ($_SERVER["REQUEST_METHOD"] === "POST") 
{
        $formObject = json_decode(file_get_contents("php://input"), false);
                ...
                if ($type == "D") {
                        $sql = "UPDATE tbl_linetoken_detail SET send_active = '0' WHERE keycode = :keycode";
                }

                if ($type == "S") {
                        $sql = "UPDATE tbl_linetoken_summary SET send_active = '0' WHERE keycode = :keycode";
                }
                ...
                try {
                        ...
                        $response = array(
                                'status' => 1,
                                'text' => "The key has been inactivated, successfully."
                        );
